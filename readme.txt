...
src/                - Holds all source code (.c files)
    ...
    graphics.c      - Provided file, with minor modifications (indicated
                      with comments)
    visible.c       - Provided file, unmodified
    wall.c          - Holds all functions that have to do with manipulating
                      walls.
    projectile.c    - Holds all functions pertaining to controlling fired
                      projectiles.
...
Wall movement can very rarely skip cycles, on the very unlikely chance that
the random number generator generates the same number many, many (>100000) times.
It has never happened yet.

It is possible to look inside a wall if you really try to, but only if you try to
nudge yourself against a wall and move tiny amounts in its direction without
actually touching the wall and triggering collision detection. Under normal
circumstances (running face-first into a wall or at a slight angle but still
towards the wall) this will not happen.

Bullets on the minimap stay on the minimap even if they leave the perimeter of the
blue walls. They will, however, disappear correctly when they eventually hit the
world border.
...
------------------------------------
Old Readme given with Assignment 1
------------------------------------
